...
class StatsRenderer:
    ...
    def strip_legend(self, chart_str):
        """
    ...
    def render_pie_chart(self, stats):
        worked = stats['worked']
        ...
        color_worked = self.config.get('color_worked', '#144620')
        color_remaining = self.config.get('color_remaining', '#6E4A02')
        color_under_target = self.config.get('color_under_target', '#8E201C')
        
        # Highlighted versions
        h_worked = self.highlight_color(color_worked)
        h_remaining = self.highlight_color(color_remaining)
        h_gap = self.highlight_color(color_under_target)
        ...
        try:
            ...
            chart_only_lines = self.strip_legend(chart_str)
            ...
            for s in original_slices:
                name = s['name']
                val = s['value']
                c_norm = self.hex_to_ansi(s['color'])
                c_high = self.hex_to_ansi(s['h_color'])
                ...
                if name == "Worked":
                    diff = worked - target
                    if diff > 0:
                        legend_lines.append(f"    (+{diff:.2f} hrs over target)")
                    elif diff < 0:
                        legend_lines.append(f"    (-{abs(diff):.2f} hrs under target)")
                    else:
                        legend_lines.append(f"    (on target)")
...
def render_pie_chart(stats, config=None):
    StatsRenderer(config).render_pie_chart(stats)

def render_summary(stats):
    StatsRenderer().render_summary(stats)
